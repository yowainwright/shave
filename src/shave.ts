export type Link = { [key: string]: string | number | boolean };
...
export default function shave(
   ...
   */
	...
	const shavedTextElAttributes = { textContent, className: charclassname };
	...
	for (let i = 0; i < els.length; i += 1) {
		...
		while (min < max) {
			...
			el[textProp] =
				spaces ? ((words.slice(0, pivot) as string[]).join(" ") as string) : (
					words as string
				).slice(0, pivot);
		...
		el[textProp] =
			spaces ? ((words.slice(0, max) as string[]).join(" ") as string) : (
				words as string
			).slice(0, max);
		el.insertAdjacentElement("beforeend", shavedTextEl);
