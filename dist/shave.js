	...
	function (global, factory) {
		typeof exports === "object" && typeof module !== "undefined"
			...
			: typeof define === "function" && define.amd
				...
				: (
					global =
						typeof globalThis !== "undefined" ? globalThis : global || self, global.shave =
						factory()
				);
	}
)(
		...
		function generateArrayOfNodes(target) {
			if (typeof target === "string") {
				return __spreadArray(
					[],
					__read(document.querySelectorAll(target)),
					false,
				);
         ...
         */
			...
			for (var i = 0; i < els.length; i += 1) {
				...
				while (min < max) {
					...
					el[textProp] =
						spaces ? words.slice(0, pivot).join(" ") : words.slice(0, pivot);
				...
				el[textProp] =
					spaces ? words.slice(0, max).join(" ") : words.slice(0, max);
				el.insertAdjacentElement("beforeend", shavedTextEl);
